...
pilimg = Image.open("/home/rmast/175789293-f39ddfdb-6f3e-4598-8d16-80a1f4a88b36-300.jpg")
print (pilimg.info['jfif_density'])
...
for x in result:
 ...
 # print(str(xc)+' '+str(yc)+' '+str(xd)+' '+str(yd)+' text')
  ...
  resized_img,ratio,topped = compute_ratio_and_resize(crop_img,width,height,80)
  ...
  #print(ret)
  #with tesserocr.PyTessBaseAPI(lang='nld+lat+Latin+eng',oem=tesserocr.OEM.DEFAULT, psm=tesserocr.PSM.SINGLE_LINE) as api:
  with tesserocr.PyTessBaseAPI(lang='nld+lat+Latin+eng',oem=tesserocr.OEM.TESSERACT_ONLY, psm=tesserocr.PSM.SINGLE_LINE) as api:

    img2 = np.uint8(resized_img) 
    im = Image.fromarray(np.uint8(resized_img))
    im.save("/home/rmast/out" + str(xc)+"-" + str(yc) +"-"+ str(width) +"-"+ str(height) + ".tif", dpi= tuple(i * topped for i in pilimg.info['jfif_density']))
    im.close()
    imggesaved = cv2.imread("/home/rmast/out" + str(xc)+"-" + str(yc) +"-"+ str(width) +"-"+ str(height) + ".tif", cv2.IMREAD_GRAYSCALE)
    print (topped)
    api.SetImage(Image.fromarray(np.uint8(imggesaved)))
    #imggesaved.close()
    ...
    if len(boxes) >0:
        for r in tesserocr.iterate_level(iterator, level):
            ...
            im.save("/home/rmast/out" + str(xc)+"-" + str(yc) +"-"+ str(width) +"-"+ str(height) +"-"+ str(bbox[0]) + ".tif", dpi= tuple(i * topped for i in pilimg.info['jfif_density']))
            im.close()
            text_list.append(symbol + " " + str(conf) + "\n")
  #          imgcv2 = cv2.imread("/home/rmast/out" + str(i) + ".tif")
  #          cv2.imshow("/home/rmast/out" + str(i) + ".tif", imgcv2)
  #          cv2.waitKey(0)
